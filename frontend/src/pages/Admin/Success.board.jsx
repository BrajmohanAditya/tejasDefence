...
import { Edit, Trash2, Users, Trophy, GraduationCap, Medal, Loader2 } from "lucide-react";
import DeleteAlertbox from "@/components/ui/DeleteAlertbox";
import SuccessBoardDialog from "../../components/Admin/success.board.jsx";
import { useGetSuccessBoardHook, useDeleteSuccessBoardHook } from "../../hooks/success.board.hook.js";

const SuccessBoard = () => {
  const { data: boardData, isLoading } = useGetSuccessBoardHook();
  const { mutate: deleteStudent, isPending: isDeleting } = useDeleteSuccessBoardHook();

  const successStudents = boardData?.students || [];
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            View, edit, and manage all selected/successful students.
          </p>
        </div>
        {/* Add Student Dialog */}
        <SuccessBoardDialog 
                  ...
                  >
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col max-w-[250px]">
                          ...
                          {item.story && (
                            <div className="text-[11px] text-slate-500 line-clamp-2 italic leading-tight mt-0.5" title={item.story}>
                            </div>
                          )}
                        </div>
                      </div>
                    ...
                    <td className="p-4 text-slate-600">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-semibold text-slate-800">{item.rank}</span>
                      </div>
                    </td>
        ...
        <div className="grid grid-cols-1 gap-4 md:hidden">
          {successStudents.map((item) => (
            ...
            >
              ...
              {item.story && (
                <div className="text-xs text-slate-600 italic line-clamp-2 bg-slate-50 p-2.5 rounded-lg border border-slate-100 relative before:content-[''] before:absolute before:left-0 before:top-2 before:bottom-2 before:w-1 before:bg-blue-200 before:rounded-r-full">
                  "{item.story}"
                </div>
              )}
