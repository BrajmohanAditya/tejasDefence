...
import { Edit, Trash2, Users, Phone, GraduationCap, Loader2 } from "lucide-react";
import DeleteAlertbox from "@/components/ui/DeleteAlertbox";
import PremiumStudentDialog from "../../components/Admin/premiumStudentDialog";
import { useGetPremiumStudentsHook, useDeletePremiumStudentHook } from "../../hooks/premium.student.hook.js";

const PremiumStudent = () => {
  const { data: studentsData, isLoading } = useGetPremiumStudentsHook();
  const { mutate: deleteStudent, isPending: isDeleting } = useDeletePremiumStudentHook();
  
  const premiumStudents = studentsData?.students || [];
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            View, edit, and manage all premium students.
          </p>
        </div>
        {/* Add Student Dialog */}
        <PremiumStudentDialog 
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-100/50 border-b border-slate-200 text-sm font-semibold text-slate-600">
                  ...
                  <th className="p-4 whitespace-nowrap">Fee Status</th>
                  <th className="p-4 text-right whitespace-nowrap">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {premiumStudents.map((item) => (
                  <tr
                    key={item._id}
                    ...
                    <td className="p-4">
                      <div className="flex flex-col gap-1">
                        <div>
                          ...
                          >
                            {item.feeStatus === "paid" ? "Paid" : "Unpaid"}
                          </span>
                        </div>
                        <div className="text-xs font-medium">
                          <span className="text-green-600">Paid: ₹{item.amountPaid}</span>
                          <span className="text-slate-300 mx-1.5">|</span>
                          <span className="text-red-500">Due: ₹{item.amountRemain}</span>
                        </div>
                      </div>
                    </td>
                ...
                {isLoading ? (
                  ...
                  <tr>
                    <td colSpan="5" className="p-12 text-center text-slate-500">
                      No students found. Click "+ Add Student" to add your first
                      one!
                    </td>
                  </tr>
                )}
        ...
        <div className="grid grid-cols-1 gap-4 md:hidden">
            ...
            >
              ...
              <div className="bg-slate-50 rounded-lg p-3 flex flex-col gap-2 text-sm border border-slate-100">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-medium text-slate-700">{item.phoneNo}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-slate-500 text-xs">Amount Paid</span>
                  <span className="font-medium text-green-600">₹{item.amountPaid}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-slate-500 text-xs">Amount Due</span>
                  <span className="font-medium text-red-600">₹{item.amountRemain}</span>
                </div>
              </div>
          ...
          {isLoading ? (
            ...
            <div className="p-8 text-center bg-white border border-dashed border-slate-300 rounded-xl text-slate-500 shadow-sm">
              No students found. Click "+ Add Student" to add your first one!
            </div>
          )}
        </div>
      </div>
